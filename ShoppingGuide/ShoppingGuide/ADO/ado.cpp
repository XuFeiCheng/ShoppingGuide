...
void CADODatabase::dump_com_error(_com_error &e)
{
	...
	ErrorStr.Format( L"CADODataBase Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n",
...
{	
	...
	m_strQuery.TrimLeft();
	BOOL bIsSelect = m_strQuery.Mid(0, wcslen(L"Select ")).CompareNoCase(L"select ") == 0 && nOption == openUnknown;
...
CString DblToStr(double dblVal, int ndigits)//保留前ndigits位到Buff
{
	CString strRet;
	char buff[50];
...
void CADORecordset::dump_com_error(_com_error &e)
{
	...
	_bstr_t bstrSource(e.Source());
	_bstr_t bstrDescription(e.Description());
	ErrorStr.Format( L"CADORecordset Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n",
		e.Error(), e.ErrorMessage(), (LPCSTR)bstrSource, (LPCSTR)bstrDescription );
...
void CADOCommand::dump_com_error(_com_error &e)
{
	...
	_bstr_t bstrSource(e.Source());
	_bstr_t bstrDescription(e.Description());
	ErrorStr.Format( L"CADOCommand Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n",
		e.Error(), e.ErrorMessage(), (LPCSTR)bstrSource, (LPCSTR)bstrDescription );
...
void CADOParameter::dump_com_error(_com_error &e)
{
	...
	_bstr_t bstrSource(e.Source());
	_bstr_t bstrDescription(e.Description());
	ErrorStr.Format( L"CADOParameter Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n",
		e.Error(), e.ErrorMessage(), (LPCSTR)bstrSource, (LPCSTR)bstrDescription );
	m_strLastError = ErrorStr;
	m_dwLastError = e.Error();
	#ifdef _DEBUG
